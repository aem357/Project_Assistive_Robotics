...
def robot_online(online):
    ...
    if online:
        robot.setConnectionParams('192.168.1.4', 30000, '/', 'anonymous', '') #ES 192.168.1.4
        time.sleep(5)
        success = robot.ConnectSafe('192.168.1.4') #ES 192.168.1.4
        time.sleep(5)
...
# Main function
def main():
    robot_online(False)  # Set to True for online mode, False for simulation
    Init()
    Pick()
    Place()
